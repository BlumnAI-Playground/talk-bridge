...
import { cliRoomMessages } from './cli.js';
 ...
 * ── 수신 웹훅 핸들러 ──────────────────────────────────────────────────────
 *
 * 게이트웨이(webhook sink)가 보내는 것은 "본문 없는 신호"다. 실측 페이로드:
 *
 *   POST /webhook
 ...
 *
 * 텍스트가 없으므로 kind 가 message/agent 면 CLI 로 본문을 2차 조회해 보강한다.
 *
 * 처리 순서가 중요하다:
 *   서명검증 → (실패 401) → **즉시 200** → 그 다음에 비동기 후처리
 * 게이트웨이 타임아웃을 피하려면 CLI 조회를 200 응답 뒤로 미뤄야 한다.
 */
...
async function process$(rawBody, deliveryId) {
  ...
  switch (kind) {
    ...
    case 'agent': {
      ...
      // 본문이 payload 에 없으므로 CLI 로 최근 메시지를 끌어와 seq 로 맞춘다.
      //
...
/** CLI 로 본문을 조회해 해당 seq 의 메시지를 찾는다. */
async function enrich(userKey, seq, kind) {
  ...
  try {
    const msgs = await cliRoomMessages(userKey, 20);
