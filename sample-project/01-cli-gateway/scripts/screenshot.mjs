...
import { cliRooms, cliRoomMessages } from '../server/cli.js';
...
/** 게이트웨이가 보내는 것과 동일한 형태의 서명된 이벤트를 주입한다. */
...
// 진행중인 방과 최근 이벤트를 CLI 에서 그대로 읽어 온다(하드코딩 금지).
const rooms = await cliRooms(20);
const live = rooms.find((r) => !r.ended) || rooms[0];
if (!live) {
  console.error('상담방이 없습니다. 고객 문의가 하나라도 있어야 화면을 찍을 수 있습니다.');
  process.exit(1);
}
const recent = (await cliRoomMessages(live.userKey, 20)).slice(-3);
