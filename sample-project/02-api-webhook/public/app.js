 ...
 *
 * 서버와의 계약:
 *   GET  /api/me                          내 정보(GET /api/agent/me)
 *   GET  /api/rooms                       상담방 목록(GET /api/agent/rooms)
 *   GET  /api/rooms/:userKey/messages     대화 내용(GET /api/agent/rooms/{userKey}/messages)
 *   POST /api/send    {userKey, text}     발신(POST /api/agent/send)
 *   POST /api/end     {userKey, event?}   종료+봇전환(POST /api/agent/end-with-bot)
 *   POST /api/block   {userKey}           차단(POST /api/agent/block)
 *   GET  /api/events                      SSE — 호스티드 웹훅 수신 실시간 푸시
 */
...
function renderRooms() {
  ...
  el.roomsHint.textContent = `${state.rooms.length}개 · REST rooms 기준`;
...
async function send(text) {
  ...
  // 낙관적 표시 — 확정은 호스티드 게이트웨이가 되돌려주는 kind:"agent" echo 로 갱신된다.
...
async function boot() {
  try {
    const { me, brand, apiBase } = await api('/api/me');
    el.meta.textContent = `${brand} · ${me.scope} · REST ${apiBase}`;
  } catch (err) {
    el.meta.textContent = `API 확인 실패: ${err.message}`;
    logEvent('error', `me 실패: ${err.message}`);
  }
