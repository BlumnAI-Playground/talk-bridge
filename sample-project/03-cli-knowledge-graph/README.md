...
**이 샘플의 위치** — 03 은 **톡브릿지 CLI 로 상담 Agent 기능을 확장하고자 할 때 활용할 수 있는 샘플**입니다.
"톡브릿지가 어떻게 부가 기능인 **상담 분석 Agent** 로 확장될 수 있나" 를 보여주는 **데모성 구현**으로, 실 운영에서는 규모에 맞는 그래프 DB 와
별도 구축·운영을 권장합니다. 대신 **로컬에서 CLI 만으로 실험**할 수 있는 기능을 제공합니다 — 설치 없이 `knowledge install` 한 번으로
온디바이스 그래프가 생기고, 데모 데이터로 조회·분석 패턴을 바로 시험해 볼 수 있습니다.
