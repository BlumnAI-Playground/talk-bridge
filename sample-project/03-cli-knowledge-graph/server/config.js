...
export const config = {
  ...
  /**
   * ── 03 추가: 상담지식 그래프 ──
   * CLI `knowledge view` 로컬 웹뷰 포트 (scripts/knowledge.mjs view 용, CLI 기본 8791).
   * v1.1.0: 게이트웨이가 떠 있어도 조회 API 를 경유해 그래프가 보인다.
   */
  knowledgeViewPort: Number(process.env.TB_KNOWLEDGE_VIEW_PORT || 8791),

  /** Cypher 한 건의 타임아웃(ms). 프리셋은 보통 0.5초 안에 끝난다 */
  knowledgeTimeoutMs: Number(process.env.TB_KNOWLEDGE_TIMEOUT_MS || 20000),
};
